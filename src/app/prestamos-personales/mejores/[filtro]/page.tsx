// src/app/prestamos-personales/mejores/[filtro]/page.tsx
// Página dinámica de "Mejores" préstamos personales con filtro específico

import { Metadata } from 'next';
import Link from 'next/link';
import { ChevronRight, Banknote, CheckCircle2, Info } from 'lucide-react';
...
export async function generateStaticParams() {
  const filters = getFiltersByCategory('personal_loan');
...
export async function generateMetadata({
  ...
  const filter = getFilterDefinition('personal_loan', params.filtro);
  ...
  return {
    ...
    alternates: {
      canonical: `/prestamos-personales/mejores/${filter.slug}`,
    },
    openGraph: {
      title: filter.seoTitle,
      description: filter.seoDescription,
      url: `https://raisket.mx/prestamos-personales/mejores/${filter.slug}`,
      type: 'article',
    },
  };
}

export default async function MejoresPrestamosPPage({
  params,
}: {
  params: { filtro: string };
}) {
  ...
  // Obtener todos los productos de préstamos personales
  const allProducts = await getFinancialProducts({ category: 'personal_loan' });
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {getFiltersByCategory('personal_loan')
              .filter((f) => f.slug !== filter.slug)
              .slice(0, 6)
              .map((relatedFilter) => (
                <Link
                  key={relatedFilter.slug}
                  href={`/prestamos-personales/mejores/${relatedFilter.slug}`}
