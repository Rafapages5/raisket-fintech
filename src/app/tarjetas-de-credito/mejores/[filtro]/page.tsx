...
// Página dinámica de "Mejores" tarjetas con filtro específico

import { Metadata } from 'next';
import Link from 'next/link';
import { ChevronRight, CreditCard, CheckCircle2, Info } from 'lucide-react';
...
export async function generateStaticParams() {
  const filters = getFiltersByCategory('credit_card');
...
export async function generateMetadata({
  ...
  const filter = getFilterDefinition('credit_card', params.filtro);
  ...
  return {
    ...
    alternates: {
      canonical: `/tarjetas-de-credito/mejores/${filter.slug}`,
    },
    openGraph: {
      title: filter.seoTitle,
      description: filter.seoDescription,
      url: `https://raisket.mx/tarjetas-de-credito/mejores/${filter.slug}`,
      type: 'article',
    },
  };
}

export default async function MejoresTarjetasPage({
  params,
}: {
  params: { filtro: string };
}) {
  ...
  // Obtener todos los productos de tarjetas
  const allProducts = await getFinancialProducts({ category: 'credit_card' });
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {getFiltersByCategory('credit_card')
              .filter((f) => f.slug !== filter.slug)
              .slice(0, 6)
              .map((relatedFilter) => (
                <Link
                  key={relatedFilter.slug}
                  href={`/tarjetas-de-credito/mejores/${relatedFilter.slug}`}
