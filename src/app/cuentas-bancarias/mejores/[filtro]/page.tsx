// src/app/cuentas-bancarias/mejores/[filtro]/page.tsx
// Página dinámica de "Mejores" cuentas bancarias con filtro específico

import { Metadata } from 'next';
import Link from 'next/link';
import { ChevronRight, Building2, CheckCircle2, Info } from 'lucide-react';
...
export async function generateStaticParams() {
  const filters = getFiltersByCategory('banking');
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/cuentas-bancarias/mejores/${filter.slug}`,
    },
    openGraph: {
      title: filter.seoTitle,
      description: filter.seoDescription,
      url: `https://raisket.mx/cuentas-bancarias/mejores/${filter.slug}`,
      type: 'article',
    },
  };
}
...
export default async function MejoresCuentasPage({
  ...
  // Obtener todos los productos de cuentas bancarias
  const allProducts = await getFinancialProducts({ category: 'banking' });
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {getFiltersByCategory('banking')
              .filter((f) => f.slug !== filter.slug)
              .slice(0, 6)
              .map((relatedFilter) => (
                <Link
                  key={relatedFilter.slug}
                  href={`/cuentas-bancarias/mejores/${relatedFilter.slug}`}
